...
// Query: *[_type == "propertyPage" && defined(slug.current)] | order(name asc) {    _id,    name,    "slug": slug.current,    location,    shortDescription,    sleeps,    "coverImage": gallery[0],    uplistingPropertySlug,    uplistingPropertyId,    wholeHouseAvailabilityId,    roomTypes[] {      name,      roomId,      image,      bedConfiguration,      guests    }  }
export type PROPERTY_PAGES_QUERY_RESULT = Array<{
  ...
  wholeHouseAvailabilityId: string | null;
  roomTypes: Array<{
    name: string;
    roomId: string | null;
    image: {
...
// Query: *[_type == "partnersPage" && _id == "partnersPage"][0] {    heroBadge,    heroHeading,    heroSubtext,    heroImage,    introParagraph1,    introParagraph2,    introParagraph3,    tiersEyebrow,    tiersHeading,    tiers[] {      title,      body    },    tiersNote,    whoWeWantEyebrow,    whoWeWantHeading,    whoWeWantCategories[] {      title,      body,      tag    },    spotlightHeading,    spotlightBody,    spotlightImage,    directoryEyebrow,    directoryHeading,    becomePartnerEyebrow,    becomePartnerHeading,    becomePartnerIntro,      "seo": {    "title": coalesce(seoTitle, name, title, areaName, ""),    "description": coalesce(seoDescription, shortDescription, excerpt, ""),    "image": seoImage,    "noIndex": noIndex == true  }  }
...
// Query: *[_type == "partner"] | order(featured desc, name asc) {    _id,    name,    "slug": slug.current,    tagline,    category,    description,    href,    featured,    image,    profileHref  }
...
// Query: *[_type == "partner" && featured == true && slug.current == $slug][0] {    _id,    name,    "slug": slug.current,    tagline,    category,    description,    href,    featured,    image,    profileHref,    profileIntro,    profileBody  }
...
// Query: *[_type == "partner" && featured == true && defined(slug.current) && !defined(profileHref)][].slug.current
